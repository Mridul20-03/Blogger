...
export const updateUser = async (req, res, next) => {
  if (req.user.id !== req.params.userId) {
    return next(
      handleError(403, "You do not have permission to update this user.")
    );
  }
...
export const deleteUser = async (req, res , next) => {
    if(!req.user.isAdmin && req.user.id !== req.params.userId) {
      return next(handleError(403,'You are not allowed to delete this user'));
    }
...
export const getUsers = async (req, res, next) => {
  if(!req.user.isAdmin){
    return next(handleError(403,'You are not allowed to see all users'));
  }
  try{
